...
@implementation _SKKeyDictionary

+ (NSData *) publicKeyForUniverse:(EUniverse)universe
{
	uint8_t keyBytes[KEY_SIZE];
    
	switch (universe)
    {
		case EUniverseInvalid:
			return nil;
			
		case EUniversePublic:
		{
			uint8_t publicKey[] =
            {
			...
			memcpy(keyBytes, publicKey, KEY_SIZE);
			break;
		}
		case EUniverseBeta:
        {
			...
			memcpy(publicKey, keyBytes, KEY_SIZE);
			break;
		}
            
		case EUniverseInternal:
        {
			uint8_t publicKey[] =
            {
				0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
		...
		default:
			[NSException raise:@"Invalid operation" format:@"No known RSA public key for Steam Universe %u", universe];
	}
	return [NSData dataWithBytes:keyBytes length:KEY_SIZE];
}
